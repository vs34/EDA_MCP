...
12. **NETLIST_EXTRACTION_RULE**: Agents MUST NOT hand-write SPICE transistor netlists from scratch on the server when running Eldo simulations. Instead, follow the standard workflow: extract the SPICE netlist programmatically from the Virtuoso schematic cellview (`MCP` library), export it to the simulation directory, and wrap it using `.include "<cellName>.cir"` in the Eldo testbench deck.
